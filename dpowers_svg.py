...
#		les points
		...
		if self.width >1:
			...
			for i in range_points: self.ptx[i] /= width
		if self.height >1:
			height = float (self.height)
			for i in range_points: self.pty[i] /= height
...
#		afficher chaque elements contenu
	...
	def scale (self, x,y):
		...
		"""
		obj.scale (self, x,y)
		for shape in self.lst: shape.scale (x,y)
